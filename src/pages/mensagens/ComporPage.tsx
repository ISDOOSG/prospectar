  ...
  if (!evoConfigured && !resendConfigured) {
    return (
      <div className="space-y-6 max-w-4xl">
        <div>
          <h1 className="text-2xl font-semibold">Compor Mensagem</h1>
      ...
      {step === "select" && (
        <div className="space-y-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-muted-foreground">
                        ...
                        {contacts && contacts.length > eligibleContacts.length && (
                          <span className="text-muted-foreground/60"> ({contacts.length - eligibleContacts.length} sem {channel === "whatsapp" ? "telefone" : "email"} ou já contatados)</span>
                        )}
                      </span>
                    </div>
                  </div>
                    ...
                    <div className="space-y-1">
                        ...
                        <label key={c.id} className="flex items-center gap-3 p-2 rounded-md hover:bg-muted/50 cursor-pointer">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-muted-foreground truncate">
                              {c.title || c.company || ""}{c.city ? ` • ${c.city}` : ""} • {channel === "whatsapp" ? c.phone : (c.custom_fields?.email || c.email)}
                            </p>
                          </div>
                          {c.score > 0 && (
                      ...
                      {eligibleContacts.length === 0 && (
                        <p className="text-sm text-muted-foreground text-center py-8">
                          Nenhum contato elegível (com {channel === "whatsapp" ? "telefone" : "email"} e status diferente de contatado)
                        </p>
                      )}
                    </div>
                  </ScrollArea>
          ...
          <Button
            size="lg"
            className="w-full"
            disabled={selectedContacts.size === 0 || generatingOpeners || (channel === "whatsapp" && !evoConnected)}
                        ...
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">
                              {channel === "whatsapp" ? contact.phone : (contact.custom_fields?.email || contact.email)} • {contact.title || contact.company || contact.city}
                            </p>
                          </div>
                          <Button
                            variant="outline"
        ...
        <Card>
          <CardContent className="flex flex-col items-center py-12 gap-4">
            ...
            <div className="flex gap-3">
              ...
              <Button variant="outline" onClick={() => navigate("/mensagens/enviados")}>
                Ver Enviados
              </Button>
              {sendResults.failed > 0 && (
                <Button variant="outline" onClick={() => navigate("/mensagens/fila")} className="text-destructive">
                  Ver Falhas
                </Button>
              )}
